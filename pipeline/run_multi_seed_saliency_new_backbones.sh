#!/bin/bash
# [MỚI — mở rộng 7 backbone mới cho saliency-weighted loss ablation
# (tab:saliency-ablation), xem models/recognition_model.py]
# pipeline/run_multi_seed_saliency.sh + _extra_seeds.sh đã kiểm chứng trên 5
# backbone gốc. Mở rộng cùng lúc với các "negative-result ablation" khác để
# nhất quán.
#
# KHÔNG train lại SR (checkpoint runs/sr_improved_<student_arch>_saliency/best.pt
# và ảnh splits/sr_improved_saliency/ không phụ thuộc backbone, đã có sẵn) —
# CHỈ thêm multi-seed recognition cho 7 backbone MỚI, thẳng n=5 seed.
#
# DÙNG:
#   bash pipeline/run_multi_seed_saliency_new_backbones.sh [đường_dẫn_config]
#
# TIỀN ĐỀ:
#   - Đã chạy xong pipeline/run_multi_seed_saliency.sh (ảnh
#     splits/sr_improved_saliency/ đã có, không phụ thuộc backbone).
...
if [ ! -d "splits/${DOMAIN}" ] || [ -z "$(find "splits/${DOMAIN}" -type f -print -quit 2>/dev/null)" ]; then
    ...
    echo "     -> chạy pipeline/run_multi_seed_saliency.sh trước (sinh SR)." >&2
    MISSING=1
fi
...
echo ""
echo "HOÀN TẤT. Kết quả: $RESULTS_DIR/multi_seed_saliency_summary.csv"
