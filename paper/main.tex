...
\begin{tikzpicture}[x=1.4cm, y=1cm, font=\small]
    ...
    % diem du lieu -- do (n=1, giam co y nghia thong ke) khac mau voi cac diem con lai (n.s. so voi 6 khoi)
    \fill[red!70!black] (0.0,1.568) circle (2.2pt);
    \foreach \x/\y in {1.4/2.149, 2.8/2.632, 4.2/3.171, 5.6/2.912, 7.0/2.758} {
        \fill[black] (\x,\y) circle (2.2pt);
    }
\end{tikzpicture}
\caption{Downstream identity accuracy (MobileNetV2, EarVN1.0, $n=5$ seeds, error bars $=\pm 1$ std.\ dev.) as a function of retained SPAB block count. The red point (1 block) is the only configuration with a statistically significant cost relative to the 6-block reference (Table~\ref{tab:depth-sweep-pairwise}); all black points (2--5 blocks) are statistically indistinguishable from it.}
...
\begin{table}[H]
\centering
\caption{Each block count vs.\ the 6-block (uncompressed) reference, MobileNetV2, $n=5$ seeds. $p_{\text{Bonf}}$ corrected across the 5-comparison family.}
\label{tab:depth-sweep-pairwise}
\begin{tabular}{lcccc}
\toprule
Blocks vs.\ 6 & Difference & Cohen's $d$ & $p_{\text{Bonf}}$ & Verdict \\
\midrule
1 & $-$0.0170 & $-$4.59 & 0.0025 & \textbf{sig.\ (cost)} \\
2 & $-$0.0087 & $-$0.85 & 0.659  & n.s. \\
3 & $-$0.0018 & $-$0.29 & 1.0    & n.s. \\
4 & +0.0059   & 0.61    & 1.0    & n.s. \\
5 & +0.0022   & 1.10    & 0.350  & n.s. \\
\bottomrule
\end{tabular}
\end{table}

The curve is not monotonic in the range that matters: accuracy is statistically flat from 2 blocks through 6 (all four comparisons non-significant after Bonferroni correction, including 5 blocks despite a raw $p$ of 0.070 that does not survive correction), and only the most aggressive cut tested, to a single block, produces a large and statistically significant cost ($d=-4.59$, $p_{\text{Bonf}}=0.0025$). The boundary this study can locate, on this backbone, therefore sits between one and two blocks, not at three: span\_tiny's specific choice of 3 blocks is accuracy-neutral, as already shown, but so -- on this evidence -- would a 2-block cut be, a further 33\% reduction in retained blocks beyond span\_tiny. This does not change the paper's proposed configuration, since span\_tiny's 3-block design has additionally been validated across five backbones and two datasets (Table~\ref{tab:depth}, Section~\ref{sec:res-awex}) where the sweep above covers one representative backbone on EarVN1.0 only; it does, however, directly answer the compression-limit question the title and abstract raise, in place of the single 3-vs-6 comparison that could previously only answer whether one specific cut was safe.
...
\begin{table}[H]
\centering
\caption{Real low-resolution holdout, EarVN1.0 (173 images, mobilenet\_v2), $n=5$ seeds.}
...
\begin{table}[H]
\centering
\caption{Real low-resolution holdout, pairwise comparisons ($d$, $p_{\text{Bonf}}$; family of 3 comparisons per metric), $n=5$ seeds.}
...
\begin{table}[H]
\centering
\caption{Real low-resolution holdout under degradation-augmented training vs.\ the original bicubic-only recipe (Table~\ref{tab:lr-holdout}), EarVN1.0 (mobilenet\_v2), $n{=}5$ seeds.}
...
\begin{table}[H]
\centering
\caption{Real low-resolution holdout under degradation-augmented training, pairwise comparisons ($d$, $p_{\text{Bonf}}$; family of 3 per metric), $n{=}5$ seeds.}
