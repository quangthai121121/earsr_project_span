...
\begin{abstract}
Ear recognition is gaining attention as a complement to face recognition, but ear images in practice are often captured at very low resolution, well below the input regime lightweight super-resolution (SR) architectures such as SPAN were designed for, and how far such an architecture can be compressed before it stops helping downstream recognition is unknown. This paper proposes \textbf{span\_tiny}, a depth-compressed variant of SPAN (3 attention blocks instead of 6, a 46.0\% parameter reduction), validated directly against downstream identity recognition rather than pixel-fidelity metrics alone. Across five recognition backbones, with multi-seed paired statistical testing including TOST equivalence tests, both span\_tiny and the uncompressed baseline significantly improve identity accuracy over no-SR on the primary dataset (EarVN1.0), while the accuracy cost of compression itself is detected in only one of five backbones. A depth ablation and a full block-count sweep confirm that this cost is genuinely negligible in most backbones, not merely undetected, with a confirmed safe floor at span\_tiny's own 3-block design. The same compression-safety finding replicates on a second, independent dataset (AWEx, 336 subjects) across three evaluation protocols, with zero of five backbones showing a significant cost under any of them. Against five other lightweight SR architectures trained under an identical recipe, span\_tiny is never significantly beaten on downstream accuracy despite ranking low on raw PSNR/SSIM, evidence that pixel-fidelity metrics are a poor proxy for recognition-relevant quality at this compression level. Three further mechanisms proposed to improve span\_tiny, and a candidate explanation for its depth tolerance, are each tested to the same standard and found not to help, reported as negative results rather than partial successes. A boundary condition is also identified: on genuinely low-resolution real-world images, rather than synthetically downsampled ones, the ordering reverses, traced to a resolution mismatch with the degradation used during training.
...
This paper's central contribution is \textbf{span\_tiny}, a depth-compressed variant of SPAN (\texttt{n\_blocks} reduced from 6 to 3, \texttt{feat}=48) that cuts parameter count by 46.0\% (0.230M vs.\ 0.4263M) and lowers inference latency correspondingly; the re-implementation is validated against the original authors' published SPAN-S results before being applied to the new task, matching 99.93\% of the reported benchmark PSNR. Three further results carry most of the paper's evidentiary weight. This paper asks a single, pre-registered question: does depth compression preserve the ordering \texttt{no-SR $<$ span\_tiny $<$ uncompressed baseline}? Multi-seed ($n=5$), multi-backbone (5 recognition architectures) statistical testing shows that both span\_tiny and the uncompressed baseline beat no-SR significantly in all five backbones on the primary dataset, EarVN1.0, while the accuracy cost of compression itself reaches significance in only one. The same compression-safety finding holds on a second, independent dataset (AWEx, 336 subjects) across three separate evaluation protocols (from-scratch training, full-fine-tuning transfer from EarVN1.0, and frozen-backbone transfer), with no backbone showing a significant accuracy cost under any of the three, evidence of generalization beyond a single dataset or transfer strategy; the secondary question of whether SR helps at all over no-SR is, by contrast, less consistently powered on this smaller dataset, and this is reported directly rather than smoothed over. The strongest single piece of evidence for the central claim comes from a depth ablation that isolates block count alone (span\_tiny against span\_large, an otherwise identical, uncompressed-depth counterpart), which shows no statistically significant difference in four of five backbones on EarVN1.0 (the fifth, EfficientNet-B0, shows a small but significant cost under one percentage point) and in all five on AWEx.
...
For every comparison between two SR domains (e.g., span\_tiny vs.\ lr), downstream recognition accuracy is measured across $n=5$ random seeds (42, 123, 2024, 44, 999), holding the SR model itself fixed. SR models are trained once per configuration, at a fixed seed of 42, and only the downstream recognition training is repeated across seeds for the main result tables; training a full SR model at every recognition seed was judged computationally infeasible at the scale of this study. This leaves open whether the specific SR checkpoint used could itself be an unmeasured source of variance. Section~\ref{sec:sr-seed-variance} measures this directly for the three core architectures rather than leaving it as an untested assumption. The paired difference in mean accuracy is reported alongside a paired-sample $t$-test (\texttt{scipy.stats.ttest\_rel}) and a paired effect size (Cohen's $d_z = \text{mean(difference)}/\text{std(difference)}$)~\cite{cohen1988statistical}. Because multiple backbones and multiple pairwise comparisons are tested within each result table, a Bonferroni correction~\cite{dunn1961multiple} is applied across the comparisons within each family. $p_{\text{Bonf}} < 0.05$ is adopted as the threshold for a ``statistically significant'' claim, with $0.05 \leq p_{\text{Bonf}} < 0.10$ distinguished as ``suggestive of a trend'' and never described as significant in this paper.

A non-significant paired $t$-test is not, by itself, evidence that two configurations are equivalent: at $n=5$ seeds with Bonferroni correction, statistical power is low, and failing to reject the null hypothesis of no difference is equally consistent with a true absence of effect and with an underpowered test that simply could not detect a real one. Wherever this paper describes a comparison as safe or accuracy-neutral rather than merely non-significant, that claim is backed by an equivalence test rather than the absence of a significant difference alone: a paired two one-sided tests (TOST) procedure~\cite{schuirmann1987tost,lakens2017equivalence}, using a pre-specified equivalence margin of $\pm 1$ percentage point of accuracy, chosen to match the informal threshold (``under one percentage point'') this paper already uses elsewhere to describe a cost as small. TOST tests, in each one-sided direction, whether the true paired difference exceeds the margin; declaring equivalence requires rejecting both one-sided nulls simultaneously ($p_{\text{TOST}} = \max(p_{\text{lower}}, p_{\text{upper}}) < 0.05$), so a comparison can simultaneously fail to reach significance under the $t$-test above and fail to be confirmed equivalent under TOST; this combination is reported explicitly as inconclusive rather than folded into either a ``significant'' or a ``safe'' verdict.

\subsection{SR-training-seed variance validation}
\label{sec:sr-seed-variance}

The multi-seed protocol above holds the SR model fixed at one checkpoint (seed 42) and repeats only the downstream recognition training. To check whether that checkpoint's own training seed is itself a meaningful source of variance (one that the downstream-seed variance already reported would not capture), span\_tiny, span\_baseline, and span\_large were each retrained at four further SR-training seeds (123, 2024, 44, 999), with the downstream recognition seed held fixed at 42 to isolate the SR-training-seed axis from the already-measured downstream-seed axis, across all five recognition backbones rather than a single representative one. Table~\ref{tab:sr-seed-variance} reports, for every backbone and architecture, the ratio of the resulting standard deviation across SR-training seeds to the standard deviation already reported across downstream-recognition seeds for the same configuration.

\begin{table}[H]
\centering
\caption{Ratio of the SR-training-seed standard deviation in downstream identity accuracy to the downstream-recognition-seed standard deviation already reported for the same configuration, EarVN1.0, $n{=}5$ SR-training seeds, all five recognition backbones.}
...
Two things follow from the full backbone coverage that a single-backbone screen could not have shown. First, the effect is not specific to span\_baseline: span\_tiny itself shows a comparable or larger ratio on two of five backbones (EfficientNet-B0, 2.44; MobileNetV2, 1.85), so this is not a limitation isolated to the comparison baseline. Second, the pattern tracks the recognition backbone at least as much as the SR architecture: EfficientNet-B0 shows an elevated ratio for span\_tiny and span\_baseline alike (but not span\_large), while GhostNet-100's highest ratio occurs under span\_large, rather than any one SR architecture being uniformly more SR-seed-sensitive than the others.

For Table~\ref{tab:main}'s central comparison specifically (span\_tiny vs.\ span\_baseline), a fully crossed design was additionally collected (all five SR-training seeds against all five downstream-recognition seeds, all five backbones, both domains, $5\times5\times5\times2=250$ observations) and analyzed with a linear mixed-effects model (\texttt{statsmodels} \texttt{MixedLM}, REML) with crossed random intercepts for SR-training seed and downstream-recognition seed (each nested within domain, since a shared seed label across span\_tiny and span\_baseline reflects no shared randomness) rather than the descriptive standard-deviation ratio used above. This separates the two variance sources explicitly instead of comparing two independently estimated standard deviations: the model attributes 32.2\% of total variance to the SR-training seed, 1.0\% to the downstream-recognition seed, and the remaining 66.9\% to residual (backbone- and domain-adjusted) variation, consistent in direction with Table~\ref{tab:sr-seed-variance}'s per-backbone ratios (SR-training-seed variance exceeding downstream-seed variance in 13 of 15 backbone/architecture combinations), though the fit reports a boundary-condition warning (the downstream-seed component is pushed toward the edge of the parameter space), so the resulting 34:1 ratio should be read as confirming the direction and rough scale of the effect rather than as a precise estimate.

An initial screening pass restricted to mobilenet\_v2 alone had suggested this was a property specific to span\_baseline (ratio 3.6$\times$ at $n=3$, 2.71$\times$ at $n=5$), with span\_tiny and span\_large both close to parity (1.16 and 0.91). Extending the check to all five backbones (Table~\ref{tab:sr-seed-variance}) does not support that reading: span\_tiny itself reaches a comparable or larger ratio on two of the five backbones, and the architecture with the highest ratio changes depending on which backbone is used for recognition. The pattern is better described as backbone-dependent (EfficientNet-B0 elevated under span\_tiny and span\_baseline; GhostNet-100 elevated under span\_large) than as a defect specific to any one SR architecture, and no confirmed mechanism is offered for why particular backbone/architecture combinations are more SR-seed-sensitive than others. span\_baseline's fixed pretrained initialization (as opposed to span\_tiny's and span\_large's training from scratch) removes one nominal source of seed-dependence rather than adding one, so a simple weight-initialization account does not explain the pattern either, and it is left as an open question. This does not change any conclusion resting on span\_tiny specifically, since its point estimates in Table~\ref{tab:main} are unaffected, but it means the extra source of variance identified here applies to the pipeline generally rather than to the comparison baseline alone, and is discussed further in Section~\ref{sec:limitations}.
...
\texttt{span\_tiny $>$ lr} reaches statistical significance ($p_{\text{Bonf}} < 0.05$) in 5/5 backbones, with large effect sizes throughout (Cohen's $d$ 2.2--9.1). \texttt{span\_baseline $>$ lr} is equally strong and significant in 5/5 backbones ($d$ 3.8--10.6, not tabulated separately). The accuracy cost of compression itself, \texttt{span\_tiny} vs.\ \texttt{span\_baseline}, reaches significance in only 1/5 backbones (EfficientNet-B0, a 1.2-percentage-point cost); the other four are statistically indistinguishable, and MobileNetV3-Small shows span\_tiny fractionally \emph{higher} than the baseline (not significant). A non-significant paired $t$-test is not, on its own, proof that the cost is negligible, so each of these four was additionally tested for equivalence (TOST, $\pm 1$ percentage point margin, Section~\ref{sec:stats}): none reaches the threshold for confirmed equivalence at $n=5$ seeds ($p_{\text{TOST}}$ 0.07--0.50), MobileNetV3-Small coming closest. This paper therefore reports the compression cost honestly as \emph{not detected as significant} in four of five backbones (a real and useful finding, since it means no measured evidence points to a cost there) rather than as \emph{confirmed accuracy-neutral}, which the present sample size cannot establish; the depth ablation below (Table~\ref{tab:depth}), which isolates the same question with a cleaner single-variable comparison, does confirm genuine equivalence in two backbones.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.60\textwidth]{figures/sample_06.png}\\[0.5mm]
{\footnotesize (a) baseline 28.654\,dB/SSIM 0.8968, span\_tiny 28.716\,dB/SSIM 0.8824: span\_tiny marginally ahead on PSNR}\\[1.5mm]
\includegraphics[width=0.60\textwidth]{figures/sample_03.png}\\[0.5mm]
...
\includegraphics[width=0.60\textwidth]{figures/sample_02.png}\\[0.5mm]
...
\includegraphics[width=0.60\textwidth]{figures/sample_18.png}\\[0.5mm]
...
\includegraphics[width=0.60\textwidth]{figures/sample_10.png}\\[0.5mm]
...
MobileNetV3-Small & 0.4423 & 0.4473 & +0.0050 & 0.37 & 1.0    & 0.226 (not equiv.) \\
ResNet-18         & 0.4825 & 0.4863 & +0.0038 & 0.45 & 1.0    & 0.371 (not equiv.) \\
\bottomrule
\end{tabular}%
}
\end{table}

4/5 backbones show no statistically significant difference between span\_tiny (0.230M parameters) and span\_large (0.417M parameters). The fifth, EfficientNet-B0, shows a small but statistically significant cost ($p_{\text{Bonf}}{=}0.0125$), span\_large ahead by 0.83 percentage points, and this cost is also not confirmed within the equivalence margin by TOST ($p_{\text{TOST}}{=}0.11$), a consistent, real (if small) cost on this backbone by both tests. Of the remaining four, TOST confirms genuine equivalence within $\pm 1$ percentage point for two (GhostNet-100, $p_{\text{TOST}}{=}0.0004$; MobileNetV2, $p_{\text{TOST}}{=}0.0235$); the other two (MobileNetV3-Small, ResNet-18) are non-significant under the paired $t$-test but do not reach the TOST threshold either, and are reported as inconclusive rather than assumed safe. The paper's central claim is therefore precisely this: reducing depth from 6 to 3 SPABs is confirmed accuracy-neutral in 2/5 backbones, shows a small but real cost in 1/5, and remains an open question at this sample size in the remaining 2/5, not a blanket claim of zero cost everywhere, and stronger where it is made than a non-significance argument alone would support. On AWEx (Section~\ref{sec:res-awex}), the same comparison is statistically non-significant in 5/5 backbones, though at a smaller sample ($n=3$ seeds; see Section~\ref{sec:limitations}).

\subsubsection{Locating the compression boundary: a full depth sweep}
\label{sec:depth-sweep}

The comparison above isolates a single point on the depth axis (3 vs.\ 6 blocks) and therefore cannot say how far compression can be pushed before it costs accuracy, only that this particular cut does not. To answer that question directly, span\_tiny's two intermediate block counts (1, 2, 4, 5) were each trained under the identical pinned recipe used for span\_tiny itself (Section~\ref{sec:recipe}, isolating block count as the only varying factor) and evaluated at $n=5$ seeds on MobileNetV2, the representative backbone used for other screening-scale checks in this study. Figure~\ref{fig:depth-sweep} plots the resulting six-point curve, Table~\ref{tab:depth-sweep} reports the same values numerically, and Table~\ref{tab:depth-sweep-pairwise} compares each compressed configuration against the uncompressed 6-block reference.

\begin{figure}[H]
\centering
\begin{tikzpicture}[x=1.4cm, y=1cm, font=\small]
    % --- trục toạ độ: x = số khối (1..6), y = accuracy (bậc thang 0.01, hệ số 70) ---
    % y_px(acc) = (acc - 0.465) * 70 ; điểm dữ liệu và lưới đều dùng chung công thức này
    \def\ymin{0}
    \def\ymax{4.2}
    \def\xmin{-0.6}
    \def\xmax{7.6}

    % lưới ngang + nhãn trục y (accuracy 0.47..0.52)
    \foreach \acc/\ypos in {0.47/0.35, 0.48/1.05, 0.49/1.75, 0.50/2.45, 0.51/3.15, 0.52/3.85} {
    ...
    % nhan truc x (so khoi 1..6) tai dung vi tri du lieu
    ...
    \node[anchor=south, rotate=90] at (\xmin-1.05,2.1) {Downstream identity accuracy};

    % duong tham chieu doc: n=3 (span_tiny, de xuat) va n=6 (span_large, uncompressed)
    \draw[dashed, blue!60!black] (2.8,\ymin) -- (2.8,\ymax);
    \node[anchor=south, blue!60!black, font=\scriptsize] at (2.8,\ymax+0.05) {span\_tiny (proposed)};
    \draw[dashed, teal!70!black] (7.0,\ymin) -- (7.0,\ymax);
    \node[anchor=south, teal!70!black, font=\scriptsize] at (7.0,\ymax+0.05) {span\_large (uncompr.)};

    % duong noi 6 diem
    \draw[thick] (0.0,1.568) -- (1.4,2.149) -- (2.8,2.632) -- (4.2,3.171) -- (5.6,2.912) -- (7.0,2.758);

    % error bar (+- 1 do lech chuan) cho tung diem, kem "mu" ngang 2 dau
    \foreach \x/\ylo/\yhi in {0.0/1.393/1.743, 1.4/1.638/2.660, 2.8/2.114/3.150, 4.2/2.548/3.794, 5.6/2.688/3.136, 7.0/2.422/3.094} {
        \draw (\x,\ylo) -- (\x,\yhi);
        \draw (\x-0.06,\ylo) -- (\x+0.06,\ylo);
        \draw (\x-0.06,\yhi) -- (\x+0.06,\yhi);
    }

    % diem du lieu -- mau theo VERDICT TOST (Table~depth-sweep-pairwise), khong chi theo NHST:
    % do = 1 khoi (sig. cost, KHONG tuong duong); xanh la = 3,5 khoi (equiv. da xac nhan qua TOST);
    % cam = 2,4 khoi (n.s. nhung KHONG xac nhan duoc tuong duong -- inconclusive, khac han "an toan")
    \fill[red!70!black] (0.0,1.568) circle (2.2pt);
    \fill[orange!85!black] (1.4,2.149) circle (2.2pt);
    \fill[green!45!black] (2.8,2.632) circle (2.2pt);
    \fill[orange!85!black] (4.2,3.171) circle (2.2pt);
    \fill[green!45!black] (5.6,2.912) circle (2.2pt);
    \fill[black] (7.0,2.758) circle (2.2pt);
\end{tikzpicture}
\caption{Downstream identity accuracy (MobileNetV2, EarVN1.0, $n=5$ seeds, error bars $=\pm 1$ std.\ dev.) as a function of retained SPAB block count, colored by verdict against the 6-block reference (Table~\ref{tab:depth-sweep-pairwise}): red (1 block) is a statistically significant, TOST-confirmed non-equivalent cost; green (3, 5 blocks) is TOST-confirmed equivalent within $\pm 1$ percentage point; orange (2, 4 blocks) is non-significant under the paired $t$-test but \emph{not} TOST-confirmed equivalent either, and is reported as inconclusive rather than safe (Section~\ref{sec:stats}).}
\label{fig:depth-sweep}
\end{figure}

\begin{table}[H]
\centering
\caption{Downstream identity accuracy across block counts 1--6, EarVN1.0 (MobileNetV2), $n=5$ seeds. 3 blocks = span\_tiny, 6 blocks = span\_large; both reproduced from Table~\ref{tab:depth}.}
\label{tab:depth-sweep}
\begin{tabular}{lcccccc}
\toprule
Blocks & 1 & 2 & 3 & 4 & 5 & 6 \\
\midrule
Accuracy & 0.4874 & 0.4957 & 0.5026 & 0.5103 & 0.5066 & 0.5044 \\
Std.\ dev.\ & 0.0025 & 0.0073 & 0.0074 & 0.0089 & 0.0032 & 0.0048 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Each block count vs.\ the 6-block (uncompressed) reference, MobileNetV2, $n=5$ seeds. $p_{\text{Bonf}}$ corrected across the 5-comparison family. $p_{\text{TOST}}$ is the equivalence test ($\pm 1$ percentage point margin, Section~\ref{sec:stats}); ``n.s.'' and ``equiv.'' are distinct claims; non-significance alone does not establish equivalence.}
\label{tab:depth-sweep-pairwise}
\begin{tabular}{lccccc}
\toprule
Blocks vs.\ 6 & Difference & Cohen's $d$ & $p_{\text{Bonf}}$ & $p_{\text{TOST}}$ & Verdict \\
\midrule
1 & $-$0.0170 & $-$4.59 & 0.0025 & 0.993 & \textbf{sig.\ cost, not equiv.} \\
2 & $-$0.0087 & $-$0.85 & 0.659  & 0.396 & n.s., not equiv.\ (inconclusive) \\
3 & $-$0.0018 & $-$0.29 & 1.0    & \textbf{0.024} & n.s., \textbf{equiv.} \\
4 & +0.0059   & 0.61    & 1.0    & 0.199 & n.s., not equiv.\ (inconclusive) \\
5 & +0.0022   & 1.10    & 0.350  & \textbf{0.0005} & n.s., \textbf{equiv.} \\
\bottomrule
\end{tabular}
\end{table}

The curve is not monotonic in the range that matters: none of 2, 3, 4, or 5 blocks reaches statistical significance against the 6-block reference after Bonferroni correction, and only the most aggressive cut tested, to a single block, produces a large and statistically significant cost ($d=-4.59$, $p_{\text{Bonf}}=0.0025$) that TOST also confirms is not equivalent ($p_{\text{TOST}}=0.993$). Non-significance alone, however, does not establish that a cut is safe (Section~\ref{sec:stats}), and TOST distinguishes two different situations among the four non-significant points: 3 and 5 blocks are confirmed equivalent to the 6-block reference within $\pm 1$ percentage point ($p_{\text{TOST}}=0.024$ and $0.0005$), while 2 and 4 blocks are neither significantly different nor confirmed equivalent ($p_{\text{TOST}}=0.396$ and $0.199$) and are reported as inconclusive at this sample size rather than assumed safe. The confirmed floor of safe compression this study can establish, on this backbone, is therefore 3 blocks (span\_tiny's own choice), not a lower one: the evidence that a 2-block cut is also safe is currently absent rather than negative, and would require more seeds to resolve rather than being assumed from the non-significant $p$-value alone. This does not change the paper's proposed configuration, since span\_tiny's 3-block design has additionally been validated across five backbones and two datasets (Table~\ref{tab:depth}, Section~\ref{sec:res-awex}) where the sweep above covers one representative backbone on EarVN1.0 only; it does, however, directly answer the compression-limit question the title and abstract raise, with an equivalence-tested floor rather than a single untested 3-vs-6 comparison.
...
\begin{table}[H]
\centering
\caption{Downstream accuracy, all pairwise comparisons among four training-loss configurations, EarVN1.0, $n=5$ seeds, three recognition backbones. Difference convention: ``A vs.\ B'' $=$ B minus A; $p_{\text{Bonf}}$ corrected across all 6 comparisons within each backbone. ``equiv.'' indicates TOST-confirmed equivalence within $\pm 1$ percentage point.}
\label{tab:loss}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccc}
\toprule
Comparison & MobileNetV2 & EfficientNet-B0 & ResNet-18 \\
\midrule
pixel+distill+identity vs.\ pixel+identity & $d{=}{-}0.84$, n.s.\ & $d{=}{-}1.01$, n.s.\ & $d{=}{-}1.06$, n.s.\ \\
pixel+distill+identity vs.\ pixel+distill  & $d{=}{+}1.93$, trend & $d{=}{+}0.33$, n.s.\ & $d{=}{-}0.24$, n.s.\ (equiv., $p_{\text{TOST}}{=}0.031$) \\
pixel+distill+identity vs.\ pixel-only     & $d{=}{+}3.13$, \textbf{sig.} & $d{=}{+}0.19$, n.s.\ & $d{=}{-}0.07$, n.s.\ \\
pixel+identity vs.\ pixel+distill          & $d{=}{+}2.85$, \textbf{sig.} & $d{=}{+}1.79$, trend & $d{=}{+}1.18$, n.s.\ \\
pixel+identity vs.\ pixel-only             & $d{=}{+}2.61$, \textbf{sig.} & $d{=}{+}1.34$, n.s.\ & $d{=}{+}1.35$, n.s.\ \\
pixel+distill vs.\ pixel-only              & $d{=}{-}0.40$, n.s.\ & $d{=}{-}0.04$, n.s.\ (equiv., $p_{\text{TOST}}{=}0.044$) & $d{=}{+}0.05$, n.s.\ \\
\bottomrule
\end{tabular}
}
\end{table}

On MobileNetV2, the six configurations resolve into two statistically-tied clusters: \{pixel+distill, pixel-only\} both significantly or near-significantly outperform \{pixel+distill+identity, pixel+identity\} (3/4 cross-cluster comparisons significant, 1/4 at trend level), while the two within-cluster comparisons are both non-significant. Extending this ablation to two further backbones, however, shows this pattern is backbone-dependent rather than general: on EfficientNet-B0 only one of the six comparisons reaches trend level and none reaches significance, and on ResNet-18 none of the six reaches even trend level, with two of the six additionally TOST-confirmed equivalent (pixel+distill+identity vs.\ pixel+distill, and pixel+distill vs.\ pixel-only). The standalone $\lambda_{\text{identity}}$ sweep (Section~\ref{sec:recipe}, also MobileNetV2-only) shows the same consistent negative direction with the same backbone-specific caveat. The identity-loss term therefore reliably hurts downstream accuracy on MobileNetV2 specifically, and this should not be read as a general property of the identity-loss mechanism across recognition backbones. By contrast, the distillation term's own marginal contribution (pixel+distill vs.\ pixel-only) remains statistically indistinguishable from pixel-only training on all three backbones, and is TOST-confirmed equivalent on EfficientNet-B0; this specific negative result is unchanged from earlier analysis and is reported as such, without reinterpretation.
...
Across all five backbones, span\_tiny is never significantly beaten by any of the four architectures compared. Against RLFN-adapted and SAFMN, the two architectures that led on raw PSNR (Table~\ref{tab:quality}), span\_tiny is statistically tied in 5/5 backbones, and is nominally ahead (though not significantly) in 3/5 backbones against SAFMN. Equivalence testing (TOST, $\pm 1$ percentage point margin) distinguishes these two ties: against SAFMN, 3 of the 5 tied backbones (GhostNet-100, MobileNetV2, ResNet-18) are TOST-confirmed genuinely equivalent ($p_{\text{TOST}}$ 0.003--0.04); against RLFN-adapted, none of the 5 tied backbones is TOST-confirmed, so that comparison remains an open, underpowered question rather than a confirmed tie. Against ECBSR, span\_tiny wins with significance in 4/5 backbones and at trend level in the fifth (ResNet-18, $p_{\text{Bonf}}{=}0.0862$). Against SMFANet, span\_tiny wins with significance in 2/5, at trend level in 1/5 (MobileNetV2, $p_{\text{Bonf}}{=}0.0577$), and is tied (nominally ahead) in the remaining 2/5. This is a materially different picture from raw PSNR/SSIM ranking, where span\_tiny placed third-lowest: on the metric the paper is actually organized around, span\_tiny is competitive with the best-performing alternatives and significantly outperforms the two weakest ones. This reinforces the point made in Section~\ref{sec:related-sr} and revisited in the qualitative analysis (Section~\ref{sec:res-main}): pixel-fidelity metrics are an imperfect proxy for recognition-relevant SR quality at this compression level. This comparison remains contextual to the paper's central claim (Sections~\ref{sec:res-main}--\ref{sec:res-depth}), which concerns depth compression \emph{within} the SPAN family, not a claim that span\_tiny is the best-performing lightweight SR architecture in absolute terms.

\subsection{Auxiliary validation: real low-resolution holdout}

On a held-out set of 173 genuinely low-resolution ear images (rather than synthetically downsampled ones), downstream accuracy \emph{reverses} the main result's ordering. Table~\ref{tab:lr-holdout} reports this at $n=5$ seeds (mobilenet\_v2), extending an earlier single-seed screening of the same holdout set.

\begin{table}[H]
\centering
\caption{Real low-resolution holdout, EarVN1.0 (173 images, mobilenet\_v2), $n=5$ seeds. Gender accuracy uses EarVN1.0's folder-range gender convention (Section~\ref{sec:datasets}), not an independently verified attribute.}
\label{tab:lr-holdout}
\begin{tabular}{lcc}
\toprule
Condition & Identity accuracy & Gender accuracy \\
\midrule
no-SR            & 0.1757 $\pm$ 0.0194 & 0.8451 $\pm$ 0.0103 \\
span\_baseline   & 0.1480 $\pm$ 0.0194 & 0.8301 $\pm$ 0.0113 \\
span\_tiny       & 0.1538 $\pm$ 0.0264 & 0.8312 $\pm$ 0.0155 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Real low-resolution holdout, pairwise comparisons ($d$, $p_{\text{Bonf}}$; family of 3 comparisons per metric), $n=5$ seeds. Gender accuracy uses EarVN1.0's folder-range gender convention (Section~\ref{sec:datasets}), not an independently verified attribute.}
\label{tab:lr-holdout-pairwise}
\begin{tabular}{lcc}
\toprule
Comparison & Identity accuracy & Gender accuracy \\
\midrule
span\_baseline $-$ no-SR & $d{=}{-}0.79$, $p{=}0.453$ (n.s.) & $d{=}{-}1.43$, $p{=}0.099$ (sig.) \\
span\_tiny $-$ no-SR     & $d{=}{-}0.96$, $p{=}0.296$ (n.s.) & $d{=}{-}0.68$, $p{=}0.603$ (n.s.) \\
span\_tiny $-$ span\_baseline & $d{=}0.14$, $p{=}1.0$ (n.s.) & $d{=}0.04$, $p{=}1.0$ (n.s.) \\
\bottomrule
\end{tabular}
\end{table}

The reversal replicates consistently in direction across all 5 seeds for both metrics: no-SR scores highest and span\_baseline lowest in every one of the 5 seeds on both identity and gender accuracy, with span\_tiny in between. On gender accuracy, span\_baseline's cost relative to no-SR reaches significance after Bonferroni correction ($d{=}{-}1.43$, $p_{\text{Bonf}}{=}0.099$); the corresponding identity-accuracy comparison does not ($d{=}{-}0.79$, $p_{\text{Bonf}}{=}0.453$), though it points the same direction with a comparable effect size. Two independent tasks (identity and gender classification) reversing in the same direction across 5 independent seeds argues against this being sampling noise specific to one task or one seed, even though $n{=}5$ is underpowered to establish significance on every individual comparison (MDES at $\alpha{=}.10$ requires $d{\geq}1.36$; the largest observed magnitude, $|d|{=}0.96$ for identity accuracy, falls short of it). span\_tiny vs.\ span\_baseline itself remains non-significant on both metrics ($d{=}0.14$ and $d{=}0.04$, respectively), consistent with the paper's central claim that the specific cost of compression is near zero, rather than a separate anomaly requiring its own explanation.

The likely cause of the no-SR/SR reversal itself is a resolution mismatch: every one of the 173 holdout images has a native short side strictly below 20\,px (range 13--19\,px, mean 17.8\,px), below the resolution the SR models were trained to receive as input. The SR models were trained exclusively to invert one specific, clean synthetic degradation (bicubic downsampling from an 80\,px HR crop); genuinely-captured images this small do not share that degradation's statistics, and many additionally have non-square aspect ratios (mean long side 25.7\,px against a 17.8\,px short side) that cause the shared letterbox-resize routine (Section~\ref{sec:arch}) to shrink the short side further still and introduce a larger padded region before the SR model sees the image. Under this distribution mismatch, a model trained to confidently reconstruct fine detail from a specific degradation can readily hallucinate plausible-looking but incorrect high-frequency structure, actively harming downstream recognition, while plain bicubic upsampling (the no-SR path) never attempts this reconstruction and therefore cannot introduce it. This is a known failure mode in the SR literature (models trained on synthetic degradation transferring poorly to real-world degradation) and is discussed further as a boundary condition on the paper's central claim in Section~\ref{sec:limitations}, rather than treated as evidence against it: the main claim is specifically about the synthetic bicubic-downsampling regime defined in Section~\ref{sec:method}, and this auxiliary check probes a related but distinct question that the main experiments were never designed to answer.

\subsubsection{Mitigation for the boundary condition: degradation-augmented training}
\label{sec:degradation-mitigation}

Rather than leaving the resolution-mismatch boundary condition as an unaddressed limitation, a direct mitigation was implemented and tested: both span\_tiny and span\_baseline were retrained with the LR input generated by a randomized real-world-style degradation pipeline (Gaussian blur, additive noise, and JPEG compression, each resampled independently per training sample per epoch) rather than clean bicubic downsampling alone, following the classical degradation-modeling approach used in BSRGAN~\cite{zhang2021designing} rather than the more elaborate two-stage pipeline of Real-ESRGAN~\cite{wang2021realesrgan}, to keep the change tractable as a single-variable ablation. Only the training-time LR generation changes; the validation split, the distillation recipe, and the frozen teacher are all unchanged from the main recipe, isolating the effect of degradation diversity specifically. Table~\ref{tab:degradation-sanity} first checks that this retraining does not disturb the paper's central claim: the paired $t$-test finds no significant difference between span\_tiny and span\_baseline under the new recipe ($d{=}{-}0.09$, $p_{\text{Bonf}}{=}0.848$), though TOST does not confirm this as equivalent within $\pm 1$ percentage point either ($p_{\text{TOST}}{=}0.097$, just short of the 0.05 threshold), consistent with the same four EarVN1.0 backbones in Table~\ref{tab:main} under the original recipe, where equivalence is likewise not confirmed at $n=5$. This sanity check is read the same way as that main comparison: no evidence of a cost under the new recipe, short of a fully confirmed equivalence claim, at a modest cost in absolute accuracy on the clean synthetic benchmark relative to the original bicubic-only recipe (mobilenet\_v2: 49.92\% and 49.80\% here vs.\ 51.25\% and 50.26\% in Table~\ref{tab:main}), an expected accuracy/robustness trade-off from training on a harder, more varied degradation distribution.

\begin{table}[H]
\centering
\caption{Compression-safety sanity check under degradation-augmented training, EarVN1.0 (mobilenet\_v2), $n{=}5$ seeds. $p_{\text{TOST}}$ is the equivalence test ($\pm 1$ percentage point margin, Section~\ref{sec:stats}).}
\label{tab:degradation-sanity}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccc}
\toprule
span\_baseline (degradation-aug.\ training) & span\_tiny (degradation-aug.\ training) & $d$, $p_{\text{Bonf}}$ & $p_{\text{TOST}}$ \\
\midrule
0.4992 $\pm$ 0.0114 & 0.4980 $\pm$ 0.0016 & $d{=}{-}0.09$, $p{=}0.848$ (n.s.) & 0.097 (not equiv.) \\
\bottomrule
\end{tabular}
}
\end{table}

Table~\ref{tab:degradation-holdout} re-evaluates both retrained models on the same 173-image real-low-resolution holdout, at $n{=}5$ seeds.
...
span\_tiny $-$ span\_baseline & $d{=}0.82$, $p{=}0.421$ (n.s.) & $d{=}1.45$, $p{=}0.095$ (sig., span\_tiny ahead) \\
\bottomrule
\end{tabular}
}
\end{table}

The effect of degradation-augmented training is asymmetric between the two architectures, and the result is reported as such rather than smoothed into a single verdict. For span\_baseline, the mitigation does not help and, on gender accuracy, makes the gap to no-SR significantly worse than under the original recipe ($d{=}{-}3.08$, $p_{\text{Bonf}}{=}0.007$, compared to $d{=}{-}1.43$ in Table~\ref{tab:lr-holdout-pairwise}). For span\_tiny, the mitigation shows a consistent, if not fully significant, improvement on both metrics: on identity accuracy, the gap to no-SR narrows from $d{=}{-}0.96$ to $d{=}{-}0.35$; on gender accuracy, span\_tiny under degradation-augmented training is nominally \emph{ahead} of no-SR (0.8509 vs.\ 0.8451, $d{=}0.23$), a reversal of the original direction, and now significantly ahead of span\_baseline under the same recipe ($d{=}1.45$, $p_{\text{Bonf}}{=}0.095$), a comparison that was flat ($d{=}0.04$) before this mitigation. None of the individual comparisons reaches significance against no-SR after Bonferroni correction at $n{=}5$, so this is reported as a partial, directionally consistent mitigation specific to the compressed model, not a resolved boundary condition: degradation-augmented training measurably helps span\_tiny's robustness to genuinely low-resolution real images without disturbing the compression-safety claim, but does not by itself close the gap to no-SR with statistical certainty, and does not transfer the same benefit to span\_baseline under the specific degradation recipe tested here.

A candidate mechanism for the asymmetry itself (that span\_baseline's larger capacity permits more confident, and potentially more incorrect, high-frequency reconstruction under distribution shift than span\_tiny's) was tested directly with a paired Laplacian-variance analysis across all 173 holdout images, using the gain in sharpness relative to no-SR as a proxy for the amount of high-frequency content each model adds; this proxy quantifies added detail but, absent a clean HR reference for these genuinely low-resolution images, cannot on its own distinguish correct detail recovery from hallucination. The paired effect size between span\_baseline and span\_tiny is negligible under both the original recipe ($d{=}0.06$) and degradation-augmented training ($d{=}{-}0.07$), and across the four conditions, mean sharpness gain correlates positively rather than negatively with mean identity accuracy (Spearman $\rho{=}0.20$, $n{=}4$), the opposite of what the capacity/hallucination account predicts. This candidate mechanism therefore finds no support, and the cause of the span\_tiny/span\_baseline asymmetry under degradation-augmented training is left open rather than assigned an unverified explanation.
...
\begin{table}[H]
\centering
\caption{Downstream identity accuracy on AWEx, from-scratch training, $n=5$ seeds. $p_{\text{TOST}}$ is the equivalence test for tiny vs.\ baseline ($\pm 1$ percentage point margin, Section~\ref{sec:stats}); ``equiv.'' requires $p_{\text{TOST}}<0.05$, a distinct claim from ``n.s.''}
...
\midrule
EfficientNet-B0   & 0.1290 & 0.1619 & 0.1524 & sig.\ ($p{=}0.0066$)   & sig.\ ($p{=}0.0127$) & n.s.\ ($p{=}0.1266$) & 0.435, not equiv. \\
...
\begin{table}[H]
\centering
\caption{Downstream identity accuracy on AWEx, transfer-learning track (full fine-tuning), $n=5$ seeds. $p_{\text{Bonf}}$ corrected across the 3-comparison family per backbone (lr--baseline, lr--tiny, baseline--tiny), matching Table~\ref{tab:main}'s convention; only \texttt{tiny vs.\ baseline}, the comparison central to this paper's claim, is tabulated, with \texttt{baseline}$>$\texttt{lr} and \texttt{tiny}$>$\texttt{lr} discussed in the main text. $p_{\text{TOST}}$ is the equivalence test ($\pm 1$ percentage point margin, Section~\ref{sec:stats}).}
...
Under full fine-tuning, \texttt{span\_baseline} $>$ \texttt{lr} reaches significance in 4/5 backbones (all but ResNet-18) and \texttt{span\_tiny} $>$ \texttt{lr} in 2/5 (MobileNetV2, MobileNetV3-Small; $d{=}3.22$ and $3.23$). Critically for this paper's central claim, \texttt{span\_tiny} vs.\ \texttt{span\_baseline} is non-significant in 5/5 backbones, though TOST does not confirm equivalence in any of the 5, unlike the from-scratch track; the compression-safety finding replicates under this protocol at the level of ``not detected as significant,'' not ``confirmed negligible.''

\begin{table}[H]
\centering
\caption{Downstream identity accuracy on AWEx, transfer-learning track (frozen backbone), $n=5$ seeds. $p_{\text{Bonf}}$ corrected across the 3-comparison family per backbone (lr--baseline, lr--tiny, baseline--tiny), matching Table~\ref{tab:main}'s convention; only \texttt{tiny vs.\ baseline}, the comparison central to this paper's claim, is tabulated, with \texttt{baseline}$>$\texttt{lr} and \texttt{tiny}$>$\texttt{lr} discussed in the main text. $p_{\text{TOST}}$ is the equivalence test ($\pm 1$ percentage point margin, Section~\ref{sec:stats}).}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.60\textwidth]{figures_awex/sample_13.png}\\[0.5mm]
...
Rather than span\_tiny's fixed choice to keep the first three of six SPABs, this mechanism learns which blocks to keep via a differentiable gating mechanism, with a sparsity penalty pushing the model toward a target size, in the same general family as scaling-factor-based structured pruning approaches such as Network Slimming~\cite{liu2017slimming}, applied here at block granularity rather than per-channel. At the sparsity level that produces an exact match to span\_tiny's parameter count (0.230M, 3 blocks retained), Table~\ref{tab:pruning-ablation} compares the two block-selection strategies at equal size.
...
Table~\ref{tab:position-ablation} reports downstream identity accuracy for the three variants across all five recognition backbones at $n=5$ seeds.

\begin{table}[H]
\centering
\caption{Block-removal position ablation, downstream accuracy, EarVN1.0, $n=5$ seeds.}
...
The design goal of this work was not maximal downstream accuracy but a specific, pre-defined criterion: preserve the ordering \texttt{lr $<$ span\_tiny $<$ span\_baseline} while substantially reducing parameter count and latency, and in particular keep the compression-specific cost (\texttt{span\_tiny} vs.\ \texttt{span\_baseline}) small and, ideally, confirmed negligible rather than merely undetected. On EarVN1.0 this cost is non-significant in 4/5 backbones (Table~\ref{tab:main}), though TOST equivalence testing does not confirm any of those four as negligible at $n=5$; the depth-isolated comparison (Table~\ref{tab:depth}) gives the cleaner answer, non-significant in 4/5 backbones and TOST-confirmed equivalent in 2 of those 4, with the remaining 2 inconclusive rather than settled. On AWEx it is non-significant in 5/5 backbones under all three protocols tested (from-scratch, full-fine-tune transfer, and frozen-backbone transfer; Section~\ref{sec:res-awex}), the single most consistent result in the paper by the non-significance criterion alone; TOST tells a more modest story there, however, confirming genuine equivalence in only 2 of the 15 backbone$\times$protocol combinations (both on the from-scratch track), with the remaining 13 non-significant but not confirmed negligible. This forms the paper's central empirical claim, stated at the strength the evidence actually supports: no detected cost almost everywhere, confirmed negligible cost in a subset, and an honestly open question rather than an assumed safety margin in the rest.
...
Two further ablations on EarVN1.0 are validated at reduced scope. The loss-component ablation (Section~\ref{sec:res-loss}) now covers three of the five backbones (MobileNetV2, EfficientNet-B0, ResNet-18) rather than one, and this extension changes the finding's interpretation: the identity-loss term's significant cost is specific to MobileNetV2 and markedly weaker or absent on the other two, so it should not be read as a general property of the mechanism across backbones. Whether either result holds on the remaining two backbones (GhostNet-100, MobileNetV3-Small) or on AWEx remains untested, and the independent $\lambda_{\text{identity}}$ sweep (Section~\ref{sec:recipe}) still covers MobileNetV2 only. The unmodified \texttt{rlfn} baseline (distinct from \texttt{rlfn\_adapted}, the primary RLFN reference used throughout) has $n=3$ seeds at the time of writing, short of the $n=5$ used elsewhere.

Two measurement-level limitations apply throughout. Latency figures (Table~\ref{tab:quality}) come from the development machine rather than a representative resource-constrained device such as a Raspberry Pi or a phone SoC, so they should be read as relative comparisons under controlled conditions rather than deployment-ready benchmarks. PSNR/SSIM/LPIPS (Table~\ref{tab:quality}, Table~\ref{tab:awex-quality}) still come from a single training run per configuration ($n=1$), with no confidence interval. The related question of whether the \emph{downstream} multi-seed results depend on which SR-training seed happened to be used was checked directly, across all five backbones and all three architectures rather than a single screening backbone (Section~\ref{sec:sr-seed-variance}): SR-training-seed variance exceeds the already-reported downstream-recognition-seed variance in 13 of 15 backbone/architecture combinations, confirmed by a crossed mixed-effects model on Table~\ref{tab:main}'s central comparison. This is not specific to span\_baseline (span\_tiny itself shows a comparable or larger ratio on two of five backbones), so it is reported as a general property of this pipeline's SR-training stochasticity rather than a caveat attached only to the comparison baseline. It does not overturn any conclusion in this paper, since the main comparisons' point estimates are unaffected, but it does mean the reported downstream-seed standard deviations understate the total uncertainty attributable to SR-training choices, and a reader treating the reported confidence intervals as capturing all sources of variance should read this section's ratios as a correction to that assumption.

Multiple-comparison correction throughout this paper uses Bonferroni~\cite{dunn1961multiple} rather than a less conservative alternative such as Holm-Bonferroni~\cite{holm1979sequential}, which controls the same family-wise error rate with uniformly higher power. Bonferroni was retained deliberately: it is simpler to verify by hand from a single multiplier ($p_{\text{Bonf}} = \min(1, p_{\text{raw}} \times n_{\text{comparisons}})$) than a sequential procedure, and, since every reported $p_{\text{Bonf}}$ in this paper is already conservative, switching corrections could in principle move some non-significant comparisons into significance but never the reverse. This paper's non-significant findings are therefore, if anything, understated rather than overstated by this choice, and its significant findings would remain significant under the less conservative alternative.

Finally, the paper's main claim is validated specifically for the synthetic bicubic-downsampling regime defined in Section~\ref{sec:method}, and this scope should be stated plainly rather than assumed to extend further: an auxiliary check on genuinely low-resolution real-world images (Section~\ref{sec:res-main}) shows the opposite ordering, traced to a resolution mismatch between these images (all natively below the SR models' trained input size) and the training-time degradation. This is a boundary condition on where the claim has been shown to hold, not a refutation of it, but it means this paper does not provide evidence that span\_tiny or span\_baseline improves recognition on arbitrary real-world low-resolution ear images outside the synthetic-degradation regime studied here. The reversal itself is now confirmed at $n=5$ seeds and replicates in direction on two independent tasks (identity and gender accuracy), so it is not attributable to one seed or one task, but the check remains limited to a single backbone (mobilenet\_v2) and a single holdout set (173 images). A direct mitigation, retraining with degradation-augmented data (Section~\ref{sec:degradation-mitigation}), was tested rather than left for future work, and helps span\_tiny specifically (narrowing or reversing its gap to no-SR on both metrics) without disturbing the compression-safety claim, but does not reach significance against no-SR at $n{=}5$ and does not help span\_baseline under the same recipe, a partial mitigation, reported as such rather than as a closed problem.
...
This paper asked a narrower question than most efficient-SR work: not whether a compressed model is the most accurate option available, but whether compressing SPAN's depth by half still leaves a model worth using ahead of no SR at all, without paying a real accuracy price for the compression itself. Across five recognition backbones and two independent datasets, span\_tiny answers both parts of that question affirmatively, stated at the strength the evidence actually supports, distinguishing throughout between a cost that was not detected and one that was positively confirmed negligible via equivalence testing (TOST, Section~\ref{sec:stats}), rather than treating the two as interchangeable. \texttt{span\_tiny} beats \texttt{no-SR} with statistical significance in 5/5 backbones on EarVN1.0; the specific cost of compression (\texttt{span\_tiny} vs.\ \texttt{span\_baseline}) is non-significant in 4/5 backbones there and in 5/5 backbones under all three evaluation protocols tested on AWEx (from-scratch, full-fine-tune transfer, frozen-backbone transfer), the most consistently replicated finding in the paper, though TOST equivalence is not confirmed for the four non-significant EarVN1.0 backbones at $n=5$. The depth ablation isolating block count alone gives the cleaner answer: span\_tiny and its uncompressed-depth counterpart, span\_large, are statistically indistinguishable in 4/5 backbones on EarVN1.0 and 5/5 on AWEx, and TOST confirms genuine equivalence in 2 of the 4 non-significant EarVN1.0 backbones, leaving the other 2 open rather than settled. A full depth sweep across every intermediate block count locates the compression boundary itself rather than testing a single cut: only the 1-block cut produces both a significant and a TOST-confirmed non-equivalent cost, 3 and 5 blocks are TOST-confirmed equivalent to the 6-block reference, and 2 and 4 blocks remain inconclusive rather than assumed safe, placing the confirmed floor of safe compression, on the backbone examined, at span\_tiny's own 3-block choice rather than lower, and directly answering how far this architecture can be compressed rather than only whether one particular cut is safe. Against five other lightweight SR architectures, span\_tiny is never significantly beaten on downstream accuracy and significantly outperforms the two weakest of them, despite ranking low on raw PSNR/SSIM, a concrete demonstration that pixel-fidelity metrics are an imperfect stand-in for the metric this paper actually cares about.

The evidence is reported with equal clarity where it runs out or points against an earlier hypothesis. Three further mechanisms proposed to improve on span\_tiny (a richer distillation loss, a saliency-weighted loss, and learned block pruning) were each validated to the same statistical standard as the main result and are reported as negative or null, with learned pruning significantly worse than span\_tiny's fixed block selection in every backbone tested. An ablation designed to test this paper's own proposed explanation for why depth reduction is tolerable (SPAN's parameter-free attention) returned a result against that explanation, and the account is revised rather than defended. An auxiliary check on genuinely real, low-resolution images reversed the main claim's ordering, and rather than being left unexplained, this was traced to a specific, verifiable cause (a resolution mismatch between these images and the synthetic degradation used in training) and is reported as a boundary condition on the paper's scope; a direct mitigation for this boundary condition, degradation-augmented retraining, was also tested and found to partially and asymmetrically help (narrowing or reversing span\_tiny's gap to no-SR without disturbing the compression-safety claim, though not to the point of statistical significance and without the same benefit for span\_baseline) and is reported as a partial result rather than a solved problem. One anomaly remains open: ResNet-18's inconsistent secondary-question evidence on AWEx, across all three protocols, lacks a confirmed explanation. None of this changes the central result, but a reader should be able to see exactly how far the evidence goes, where a hypothesis was tested and did not survive, and where it stops.

Extending this work would involve latency measured on an actual edge device rather than development hardware, confidence intervals for the SR-quality metrics (currently single-run), extending the six-architecture context comparison and the three additional-mechanism ablations to AWEx to test whether they replicate cross-dataset, and, if a suitable third dataset becomes available, an additional independent check on generalization beyond EarVN1.0 and AWEx. Two further questions raised in Section~\ref{sec:extended-ablations} about span\_tiny's specific design choices have since been resolved rather than left open: a block-removal position ablation, testing whether the specific choice of retained blocks was well-supported, found no significant effect of removal position at $n=5$ seeds across all five backbones; and a candidate recognition-embedding-space distillation loss, motivated by the recognition-aware SR literature reviewed in Section~\ref{sec:related}, turned out on inspection to be mathematically identical to the identity-loss term already evaluated in Section~\ref{sec:res-loss}, an account that does not hold in this setting. A further question about the multi-seed protocol itself (whether fixing the SR-training seed and repeating only the downstream-recognition seed could understate the paper's true variance) was also checked directly rather than assumed away, across all five backbones and all three architectures, and confirmed with a crossed mixed-effects model on the paper's central comparison rather than a descriptive standard-deviation ratio alone (Section~\ref{sec:sr-seed-variance}): SR-training-seed variance exceeds the already-reported downstream-recognition-seed variance in most backbone/architecture combinations, including for span\_tiny, so this is reported as a general property of the pipeline rather than a caveat specific to the comparison baseline.
...
IEEE Transactions on Pattern Analysis and Machine Intelligence \textbf{38}(2), 295--307 (2016)

\bibitem{shi2016espcn}
Shi, W., Caballero, J., Husz\'ar, F., Totz, J., Aitken, A.P., Bishop, R., Rueckert, D., Wang, Z.:
Real-Time Single Image and Video Super-Resolution Using an Efficient Sub-Pixel Convolutional Neural Network.
In: Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), pp.\ 1874--1883 (2016)

\bibitem{ledig2017srgan}
Ledig, C., Theis, L., Husz\'ar, F., Caballero, J., Cunningham, A., Acosta, A., Aitken, A., Tejani, A., Totz, J., Wang, Z., Shi, W.:
Photo-Realistic Single Image Super-Resolution Using a Generative Adversarial Network.
In: Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), pp.\ 4681--4690 (2017)

\bibitem{hu2018senet}
Hu, J., Shen, L., Sun, G.:
Squeeze-and-Excitation Networks.
In: Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), pp.\ 7132--7141 (2018)

\bibitem{ding2021repvgg}
Ding, X., Zhang, X., Ma, N., Han, J., Ding, G., Sun, J.:
RepVGG: Making VGG-style ConvNets Great Again.
In: Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), pp.\ 13733--13742 (2021)
...
\bibitem{wang2016vlrr}
Wang, Z., Chang, S., Yang, Y., Liu, D., Huang, T.S.:
Studying Very Low Resolution Recognition Using Deep Networks.
In: Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), pp.\ 4792--4800 (2016)
...
In: Proceedings of the IEEE International Conference on Computer Vision (ICCV), pp.\ 2755--2763 (2017)
...
Journal of Pharmacokinetics and Biopharmaceutics \textbf{15}(6), 657--680 (1987)

\bibitem{lakens2017equivalence}
Lakens, D.:
Equivalence Tests: A Practical Primer for t Tests, Correlations, and Meta-Analyses.
Social Psychological and Personality Science \textbf{8}(4), 355--362 (2017)
...
\bibitem{zhang2021designing}
Zhang, K., Liang, J., Van Gool, L., Timofte, R.:
Designing a Practical Degradation Model for Deep Blind Image Super-Resolution.
In: Proceedings of the IEEE/CVF International Conference on Computer Vision (ICCV), pp. 4791--4800 (2021)

\bibitem{wang2021realesrgan}
Wang, X., Xie, L., Dong, C., Shan, Y.:
Real-ESRGAN: Training Real-World Blind Super-Resolution with Pure Synthetic Data.
In: Proceedings of the IEEE/CVF International Conference on Computer Vision Workshops (ICCVW), pp. 1905--1914 (2021)
